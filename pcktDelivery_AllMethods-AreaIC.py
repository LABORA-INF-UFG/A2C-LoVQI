    ...
    for method in methods:
        ...
        for g in gateways_:
            ...
            if method in ["dqn", "ddqn"]:
                ...
                ax1.fill_between(virtual_positions_non_zero,
                                 [m - c for m, c in zip(mean_rates, ci_95_intervals)],
                                 [m + c for m, c in zip(mean_rates, ci_95_intervals)],
                                 color=colors.get(method, "black"), alpha=0.3)
            else:
                ax1.plot(virtual_positions, delivery_rate_v[method][g], marker="o",
                         label=f"{labels[method]} {g} GW", color=colors.get(method, "black"), linestyle=lines[g])
                ax1.fill_between(virtual_positions,
                                 [m - c for m, c in zip(mean_rates, ci_95_intervals)],
                                 [m + c for m, c in zip(mean_rates, ci_95_intervals)],
                                 color=colors.get(method, "black"), alpha=0.3)
        ...
        for g in gateways_:
            ...
            if method in ["dqn", "ddqn"]:
                ax2.plot(devices_non_zero, mean_rates, marker="o",
                         label=f"{labels[method]} {g} GW", color=colors.get(method, "black"), linestyle=lines[g])
                ax2.fill_between(devices_non_zero,
                                 [m - c for m, c in zip(mean_rates, ci_95_intervals)],
                                 [m + c for m, c in zip(mean_rates, ci_95_intervals)],
