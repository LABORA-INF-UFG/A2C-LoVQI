...
from A2C_mapping import Actor, Critic
...
for ep in range(episodes):
    ...
    if ep % (ep // 5) == 0 or ep == episodes:
        checkpoint_name_actor = f"A2C_actor_chkpt_{ep}_{virtual_positions}V_{nGateways}G_{nDevices}d.pth"
        checkpoint_name_critic = f"A2C_critic_chkpt_{ep}_{virtual_positions}V_{nGateways}G_{nDevices}d.pth"
        if not os.path.exists(CHECKPOINT_DIR):
            os.makedirs(CHECKPOINT_DIR)
        actor_path = os.path.join(CHECKPOINT_DIR, checkpoint_name_actor)
        critic_path = os.path.join(CHECKPOINT_DIR, checkpoint_name_critic)
...
file_name = f"{path_files}A2C_results_{virtual_positions}V_{nGateways}G_{nDevices}D.dat"
with open(file_name, "w") as file:
    file.write("episodio,tempo,reward,actor_loss,critic_loss\n")
    for idx, (time_elapsed, reward, actor_loss, critic_loss) in enumerate(
            zip(execution_times, episode_mean_rewards, episode_mean_actor_loss, episode_mean_critic_loss)):
        file.write(f"{idx + 1},{time_elapsed:.4f},{reward:.4f},{actor_loss:.4f}, {critic_loss:.4f}\n")

if print_movements:
    file_name = f"{path_files}A2C_episodes_movements_{virtual_positions}V_{nGateways}G_{nDevices}D.dat"
    with open(file_name, "w") as file:
